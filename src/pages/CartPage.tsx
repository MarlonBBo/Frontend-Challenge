...
import { addDecimals, multiplyDecimal } from "@/lib/decimal"
...
function CartLoading() {
  return <div role="status" className="space-y-4 px-6 sm:px-0"><span className="sr-only">Carregando carrinho...</span>{Array.from({ length: 4 }, (_, index) => <div key={index} className="h-[92px] animate-pulse rounded-[15px] bg-[#261512]" />)}</div>
}
...
export function CartPage() {
  ...
  const subtotal = addDecimals(items.map((item) => multiplyDecimal(item.unitPriceEth, item.quantity)))
  const total = addDecimals([subtotal, "0.016"])
      ...
      {isPending ? <CartLoading /> : isError ? (
        ...
        <div className="grid items-start gap-12 xl:grid-cols-[minmax(0,2.35fr)_minmax(280px,1fr)] xl:gap-20">
          ...
          <section aria-labelledby="cart-summary-title" className="mt-2 rounded-t-[36px] bg-[#211310] px-6 pt-5 pb-6 sm:mt-0 sm:rounded-none sm:bg-transparent sm:px-0 sm:pt-3 sm:pb-0">
            ...
            <div className="flex rounded-full border border-[#D28A4C]/20 sm:mt-2 sm:rounded-none sm:border-0"><Input id="cart-coupon" disabled placeholder="Digite o código promocional..." className="h-11 min-w-0 flex-1 rounded-l-full rounded-r-none border-0 bg-transparent px-3 text-xs shadow-none placeholder:text-[#A58A58] disabled:opacity-100 sm:h-10 sm:rounded-l-sm sm:border sm:border-[#D28A4C] sm:px-2" /><Button disabled variant="kurio" className="h-11 rounded-full px-5 font-bold disabled:opacity-100 sm:h-10 sm:rounded-l-none sm:rounded-r-sm">Aplicar</Button></div>
            <dl className="mt-4 space-y-3 text-sm sm:mt-6"><div className="flex justify-between gap-3"><dt>Subtotal</dt><dd className="whitespace-nowrap text-base">{formatDecimal(subtotal)} ETH</dd></div><div className="flex justify-between gap-3"><dt>Desconto do lançamento</dt><dd className="whitespace-nowrap">(-) 00.00</dd></div><div className="flex justify-between gap-3"><dt>Taxa de rede</dt><dd className="whitespace-nowrap text-base">0.016 ETH</dd></div></dl>
            <p className="mt-2 text-right text-xs text-[#E89B55]">Taxa estimada</p>
            <div className="mt-5 flex justify-between gap-3 font-bold sm:mt-6"><span>Total</span><span className="text-[#E89B55]">{formatDecimal(total, 3)} ETH</span></div>
